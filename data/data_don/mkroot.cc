int mkroot(int don){
  ...
  tnew->Branch("donid", &donid, "donid/I"); //Tree のブランチ(枝)に donid を登録
  tnew->Branch("evid", &evid, "evid/I");//Tree のブランチ(枝)に イベント番号 を登録
  tnew->Branch("a", a, "a[1]/F"); //Tree のブランチ(枝)に ADC (a[0], a[1], ..., a[7])を登録
  tnew->Branch("t", t, "t[1]/F"); //Tree のブランチ(枝)に TDC (t[0], t[1], ..., t[7])を登録
  ...
  while(!ifile->eof()) { //ファイルの最後までループを回す
    donid = don; // donid は与えられた don 番号を代入
